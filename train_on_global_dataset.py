...
from utils import get_network, get_training_dataloader, get_valid_dataloader, get_test_dataloader, \
    most_recent_folder, most_recent_weights, last_epoch, best_auc_weights, get_parameter_number,save_best_result
import sys
import csv
...
if __name__ == '__main__':
    ...
    result_csv_dir = os.path.join(rootPath, "Global_datasets_result", "result_csv")
    ...
    for epoch in range(1, settings.EPOCH + 1):
        ...
        patience -= 1
        if patience == 0:
            print("The best:", best_testAUC)
            print("The end!")
            break

    """记录bestAUC"""
    bestAUC_csv = os.path.join(result_csv_dir, "Global_BestResult.csv")
    ...
    with open(bestAUC_csv, 'a+', newline="") as f:
        csv_write = csv.writer(f)
        data_row = ["Global", best_epoch, best_testAUC, best_testAcc, best_testPrec, best_testRecall, best_testF1]

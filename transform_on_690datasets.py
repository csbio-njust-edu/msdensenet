...
if __name__ == '__main__':
    ...
    result_csv_dir = os.path.join(rootPath, "690_datasets_result", "result_csv")
    if not os.path.exists(result_csv_dir):
        os.makedirs(result_csv_dir)

    data_index = 1
    for dataset_ in dataset_list:
        print("This is the ", data_index, " dataset")
        data_index += 1
        ...
        if args.resume:
            ...
            if best_weights:
                ...
                _, best_auc, cur_result, pred_result_test,_,_,_,_ = eval_training(net, dna_valid_loader, dna_test_loader,
        ...
        for epoch in range(1, settings.EPOCH_TRANSFORM + 1):
            ...
            if patience == 0:
                ...
                break
        """记录bestAUC"""
        bestAUC_csv = os.path.join(result_csv_dir, "690Datasets_BestResult.csv")
        if data_index == 2:
